...
namespace Common {
    namespace Errors {
        ...
        uint32_t BaseError::GetDeep() const {
            return _deep;
        }
        ...
        Str BaseError::WhatHappened() const {
            ...
            result.Replace("{module}", _module.value_or(defaultValue))
            ...
            if (_suberror)
                result.append(_delimiter.value_or(DefaultErrorsDelimiter)).append(_suberror->WhatHappened());
